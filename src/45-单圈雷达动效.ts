...
const vec3Zero = new THREE.Vector3(0, 0, 0);
    ...
    if (curFuncIndex === 3) {
        ...
        if (radarMeshArray.length === 0) {
            ...
            for (let i = 0; i < radarVertexArray.length; i++) {
                ...
                boxMesh.userData['orginPos'] = boxMesh.position.clone();
        ...
        for (let i = 0; i < radarMeshArray.length; i++) {
            if (!radarMeshArray[i].userData['isTweening']) {
                gsap.to(radarMeshArray[i].position, {
                    x: radarMeshArray[i].userData['targetPos'].x,
                    y: radarMeshArray[i].userData['targetPos'].y,
                    z: radarMeshArray[i].userData['targetPos'].z,
                    duration: 2,
                    repeat: -1,
                    ease: 'none',
                    onStart: ()=>{
                        radarMeshArray[i].userData['isTweening'] = true;
                    }
                });
            }
